...
module.exports = function(cookie) {
    login(cookie, (err, bot) => {
        if (err) Promise.resolve("An unknown error.")
                    .then(log.writeError)
                    .then(fs.unlinkSync(path.join(__dirname,'..','user','session.json')));

        bot.setOptions(option);
            bot.listen((err, msg) => {
                if (err) log.writeError("Unable to connect conversation.");
                console.log(msg);
                async function proc (msg) {
                    switch(msg.body){
                        ...
                        case "help":
                            if(msg.isGroup){
                                bot.sendMessage("The content has been sent to your mailbox",msg.threadID);
                            }
                        ...
                        nhentai.get(msg.body.slice(11,msg.body.length))
                            .then((res) => {
                                if (!res.error) {
                                    ...
                                    bot.sendMessage("error, id not defined 😞",msg.threadID);
                            })
                    }
                    ...
                    if(msg.body.indexOf("set sticker ") == 0){
                        bot.sendMessage("send a sticker ...",msg.threadID); // ezz
                    }
                    ...
                    if(msg.body.indexOf("!! ") == 0){
                        if(isAdmin(msg.senderID)){
                            ...
                            bot.sendMessage("You are not authorized to execute this command 😏👌",msg.threadID);    
                    }
                    /* ban user */
